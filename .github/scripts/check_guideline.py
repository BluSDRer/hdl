...
def check_filename (filename):
    ...
    if (filename.endswith('.v') == False):
        ok = False
    #if (filename.find("axi_adxcvr") != -1):
    #    ok = False
    #if (filename.find("clkdiv") != -1):
    #    ok = False
    #if (filename.find("ad_mul") != -1):
    #    ok = False
...
def get_and_check_module (module_path, lw):
    ...
    lw.append("\nWarnings while checking the module definition file:")
    ...
    for line in list_of_lines:
        ...
        if ((line.find("module") == -1) and (line.find("endmodule") == -1) 
            ...
            if (not (indent_nb >= 2)):
                ...
                if (indent_nb != 2 and is_paramdef(line)):#((is_iodef(line) and (line.find(";") == -1)) or is_paramdef(line))): 
                    lw.append(module_path + " : " + str(line_nb) + " indentation is not proper 2")
                    
                    ## indentation check should be avoided when:
                    ## a. multiline comment
                    #if ((line.find("/*") != -1) or (line.find("*/") != -1) 
                    #    or (line.strip()).startswith("*")):
                    #    avoid_indentation_check = True
                    ## b. when there is no ; and the line doesn't start with a dot
                    #if (line.find(";") == -1 and (not (line.strip()).startswith("."))):
                    #    avoid_indentation_check = True
                    ## c. when there is a ? in a line 
                    #if (line.find("?") != -1):
                    #    avoid_indentation_check = True
                    ## d. when there is no dot and there is a ({ or }) and  
                    #if ((line.find(".") == -1) and ((line.find("({") != -1) or (line.find("})") != -1))):
                    #    avoid_indentation_check = True
                    ## e. when it starts with a (
                    #if ((line.strip()).startswith("(")):
                    #    avoid_indentation_check = True  
        ...
        if ((is_comment(line) == False) and (name_found == False)):
            if (pos_module == 0):
                ## situations accepted
...
def check_guideline_instances (occurrence_item, lw):
    ...
    # list of warnings
    lw_initial_size = len(lw)
    lw.append("\nWarnings while checking the module instance file:")
...
###############################################################################
#
# Check guideline for files in library/
###############################################################################

error_files = []

# print all instances that were found and where they appear in other files
...
if (len(sys.argv) > 1):
            ...
            for root, dirs, files in os.walk("./"):
                for name in files:
                    if((name == sys.argv[arg_nb]) and (check_filename(name))):
                        ...
                        all_detected_modules.append(module_path)
            arg_nb += 1
        ...
        while (arg_nb < len(sys.argv)):
            if (os.path.exists(sys.argv[arg_nb])):
                if (check_filename(sys.argv[arg_nb])):
                    all_detected_modules.append(sys.argv[arg_nb])
            else:
    ...
    all_detected_modules = detect_all_modules("./")

# no matter the number of arguments
if (len(all_detected_modules) <= 0):
    print("NO detected modules")
    guideline_ok = True
else:
    for module_path in all_detected_modules:
        # list of warnings 
        lw = []
    ...
    for module_name in xilinx_modules:
        ...
        if (len(xilinx_occ_list) > 0):
            for xilinx_occ_it in xilinx_occ_list:
                ...
                for it in all_detected_modules:
